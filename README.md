...
- 取消了分片/重新组装相关字段。IPv6不允许中间路由器进行分片与充足
...
浏览器首先看一下自己的缓存里有没有，如果没有就向操作系统的缓存要，还没有就检查本机域名解析文件`hosts`，如果还没有，就会DNS服务器进行擦汗寻，查询过程如下：
...
我们的电脑通常都是通过`DHCP`(Dynamic Host Configuration Protocol，动态主机配置协议)动态获取IP地址，大大省去了配IP信息繁琐的过程。
...
网络地址转换NAT来缓解IPv4地址耗尽问题，简单来说<font color="red" size=4>NAT就是同个公司、家庭、教室内的主机对外部通信时，把私有IP地址转换成公有IP地址。</font>
